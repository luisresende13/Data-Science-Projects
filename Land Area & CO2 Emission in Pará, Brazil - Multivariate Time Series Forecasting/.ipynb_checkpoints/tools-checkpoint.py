...
class preprocess:
    ...
    def load_series(path='series/', index_col=0):
        ...
        for filename in filenames:
            ...
            series[filename[:-4]] = pd.read_csv(os.path.join(path, filename), index_col=index_col)
    ...
    def clean_series(series, train_min=1, drop_start_zeros=True, save=False, path='data/clean/series/'):
        clean_series = {}
        test_empty = []
        excluded_test_index = []
        n_total = len(series)
        ...
        if save:
            ...
            for key in clean_series.keys():
                ...
                clean_series[key].to_csv(os.path.join(path, key+'.csv'), index=True)
    ...
    ## Return series of 'target' column values with label encoded column 'index_col' as index
    ...
    ## Return dataframe with 'cols' columns with label encoded column 'index_col' values as index
