...
class ClassificationPipeline:
    ...
    def binary(
        ...
        scrs = pd.DataFrame(classification_report(ye, yhat, output_dict=True))  # Evaluate model prediction
#         scr_cols = ['0.0', '1.0', 'macro avg']
#         for col in scr_cols:
#             if col not in scrs:
#                 scrs[col] = np.nan
        if store:
...
def classesGroupRecall(scrs, cls_cnts, ignore_first=5):
    ...
    scrs['recall'].loc[:, ['0.0', '1.0', 'macro avg']].unstack('metric').sort_values('1.0').plot(use_index=False, ax=ax[0])
    ax[0].set(
    ...
    sort_scrs = scrs['recall'].loc[:, ['0.0', '1.0', 'macro avg']].unstack('metric').sort_values('macro avg')
