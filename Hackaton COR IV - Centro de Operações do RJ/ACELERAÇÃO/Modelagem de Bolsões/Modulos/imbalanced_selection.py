...
class MinorityGroupSplitUndersample:
    ...
    def __init__(self,
        n_splits=5, test_size=.2, train_size=None,
    ...
    def split(self, X, Y, groups, strategy='GroupShuffleSplit'):    
        ...
        for t_ind, e_ind in splitter.split(X[mino], Y[mino], groups[mino]):
            ...
            yt_majo = Y_majo.sample(int(len(yt_mino) * self.train_prct), random_state=self.random_state)
            ye_majo = Y_majo.drop(yt_majo.index)
            if self.test_prct is not None:
                ye_majo = ye_majo.sample(int(len(ye_mino) * self.test_prct), random_state=self.random_state)
