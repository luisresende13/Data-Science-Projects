...
def print_cls_cnt(Y, t_ind, e_ind, display_cnt=True, return_cnt=False):
    cls_cnt = pd.concat([Y[ind].value_counts().rename(col).to_frame() for ind, col in zip([t_ind, e_ind], ['Train set', 'Test set'])], 1).fillna('')
    cls_cnt.index.name = 'Class'
    if display_cnt: display(cls_cnt)
    if return_cnt: return cls_cnt
